...
install_obsidian() {
    ...
    sudo dnf install -y ./appimagelauncher_2.2.0-travis995.0f91801.bionic_amd64.deb
...
base() {
	...
	# create dnf sandbox user
	if [[ ! $(id -u $TARGET_USER) ]]; then
		echo "[!] Creating the dnf sandbox user _apt"
...
usage() {
    ...
    echo "  dev-tools                                 - install C development tools"
}
